  ...
  server.registerTool(
    'globalise_inspect_page_image',
    {
      description:
        ...
        "To read a small hand, request a TIGHTER REGION rather than a bigger size: size is clamped per page shape to what arrives intact, and anything beyond that is downscaled before it reaches the assistant, so a narrower crop raises pixels-per-letter where a larger size raises nothing. A clamp is reported in `warnings`. quality 'gray' can help with faint ink. " +
