 ...
 * is removed, not BUILD_DIR — though since the bundle filename is fixed and
 * unversioned, each pack overwrites the previous .mcpb in mcpb-build/.
 */
...
function finalizeMetadata(): void {
  ...
  // Mirror the same text to a *tracked* path so the manifest's `documentation`
  // link resolves on GitHub — mcpb-build/ is gitignored, so the staged copy
  // above never leaves this machine. Regenerated on every pack; commit it
  // alongside a release rather than hand-editing (edit bundleReadme() instead).
  writeFileSync(
    join(PACKAGE_ROOT, 'docs', 'bundle-readme.md'),
...
function packBundle(): string {
  ...
  // Fixed, unversioned filename matching the companion skill package
  // (globalise-voc-research.skill) — the GitHub release tag carries the
  // version, so the asset name stays stable across releases. NB each pack
  // therefore overwrites the previous bundle in mcpb-build/.
  const outFile = join(BUILD_DIR, 'globalise-voc-research.mcpb');
