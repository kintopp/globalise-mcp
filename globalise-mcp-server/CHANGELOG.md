...
- **(plan 005 — the material one) List-tool size-capping.** The four list tools (`search_transcriptions`, `find_archival_documents`, `lookup_commodity`, `lookup_measure`) now return **fewer rows than `size` requested** when a response exceeds the byte budget (≈60KB/copy with defaults), setting `pagination.hasMore=true` and a `Response size-capped: returned N of M…` `note` while leaving `total` untouched. SKILL.md never told the model this could happen, so a short page with `hasMore` read as a failed query rather than expected large-response behavior. Added a new **"When a response is size-capped"** section (covers both the list-tool row-drop and, per plan 006, the single-page line-trim, plus the recovery: higher `from` / narrower filters / lower `size`/`fragmentSize`), and extended the "Quick do/don't" `note`-trigger checklist (was: auto-quoted terms / phrase fallback / lower-bound totals) to include **size-capped results**.
- **(plan 004) `fragmentSize` search lever.** New caller param (20–500, default 150, was a fixed 500) controlling `highlightedFragments` snippet length — already surfaced in the always-loaded tool description (`index.ts`) but absent from the skill. Added a paragraph to "Searching transcriptions" framing it as the cheapest payload lever (multiplies by `size`) and the first thing to lower on a size-capped search.
- **(plan 006) Single-page line truncation.** `retrieve_document`/`navigate` can tail-trim a dense page's `text.lines`, signaling it via `text.truncated`/`text.totalLines`; `view_document_ui` is exempt. The seven-tools table no longer calls retrieve's output a *"full"* transcription (now imprecise in that rare case), and the new size-cap section names the `truncated` flag and points at the exempt viewer / scan link as the fallback.

Same pass also ran a **condensation** to keep SKILL.md under the ~500-line Agent-Skills guideline: the three additions had pushed it 515 → 548, so verbose blocks were tightened **prose-density only — no fact, number, or caveat removed** (Worked-patterns reflowed from question/answer pairs into one-line bullets; RGP-edition, commodity-provenance, weights-&-measures, colonial-content, and the new size-cap sections compressed), landing at **495** lines (a battery of 21 load-bearing facts re-verified present afterward). Patch bump mirroring the v2.9.2 / v2.10.2 / v2.10.3 SKILL-refinement precedent; `.skill` + `.skill.zip` regenerated and verified byte-identical to the source `SKILL.md`.

## [2.11.2] - 2026-06-26

Response-size note no longer stacks duplicate cap messages when the size-shave loop iterates. `fitResultToBudget` calls `onTrim` once for the initial estimate and again on each guard-loop iteration (to keep the byte budget honest — the loop must "see" the note's own size), but `recordListTrim.onTrim` was APPENDING its cap message to `result.note` each call, so a multi-iteration shave produced a note with two contradictory counts (e.g. "returned 96 of 500 … returned 81 of 500" — the array held the final 81; "returned 96" was stale and confusing). Fixed by capturing the note as it stands on the FIRST `onTrim` call (lazily, AFTER any `searchResultTrim.compact()` note was added) and REBUILDING from that base each call instead of appending — so repeated invocations never stack. The lazy capture preserves the `compact` snippet note for `searchResultTrim`. `documentLineTrim`/`navigateLineTrim` were already idempotent (they set the same `truncated`/`totalLines` each call) and are unchanged; `fitResultToBudget` is unchanged. `scripts/test-response-size.ts` gains exactly-once assertions on the row-drop cap message (Case 3 and Case 6b) plus a compact-note-survives check. Pure-util + test only; no schema/DB/`.skill` change.

## [2.11.1] - 2026-06-26

Line-level response trim for `globalise_retrieve_document` and `globalise_navigate`. Plan 005 bounded the four list tools; this plan closes the gap for the two single-page tools, which return `text.lines: string[]`. When the page text exceeds the response budget, tail lines are dropped and two new optional output fields signal the truncation: `text.truncated: true` and `text.totalLines: <original count>`. The metadata, navigation, and URLs blocks are never touched — only `text.lines` is trimmed. `globalise_view_document_ui` is explicitly exempt: its `structuredContent` feeds the interactive iframe and must stay complete (it uses `registerAppTool`, not `registerJsonTool`, so it has no `trimStrategy` slot). New `documentLineTrim` and `navigateLineTrim` strategies added to `src/utils/response-size.ts`; test-response-size.ts extended with 3 new cases (9 new checks). Patch bump; no DB rebuild; no `.skill` change.
